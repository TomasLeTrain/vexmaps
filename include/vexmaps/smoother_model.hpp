...
class SmootherModel : public LocalizationModel {
  private:
    ...
    Time last_global_pose_timestamp = 0_msec;
    ...
    void update() override {
        ...
        if (current_local_delta_timestamp != last_local_delta_timestamp) {
            ...
            velocity_estimate.orientation =
              velocity_estimate.orientation +
              beta_vtheta * (pose_delta_measurement.orientation /
            ...
            pose_estimate.x =
              pose_estimate.x +
              beta_x * ((pose_delta_measurement.x + pose_prediction.x) -
                        pose_estimate.x);

            pose_estimate.y =
              pose_estimate.y +
              beta_y * ((pose_delta_measurement.y + pose_prediction.y) -
                        pose_estimate.y);

            pose_estimate.orientation =
              pose_estimate.orientation +
              beta_theta * ((pose_delta_measurement.orientation +
                             pose_prediction.orientation) -
                            pose_estimate.orientation);
        ...
        if (current_pose_timestamp != last_global_pose_timestamp) {

            last_global_pose_timestamp = current_pose_timestamp;

            units::Pose pose_measurement = pose_model->getPose();

            pose_estimate.x = pose_estimate.x +
                              alpha_x * (pose_measurement.x - pose_estimate.x);

            pose_estimate.y = pose_estimate.y +
                              alpha_y * (pose_measurement.y - pose_estimate.y);
